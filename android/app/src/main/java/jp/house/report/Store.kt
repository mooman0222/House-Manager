...
object ResultStore {
    ...
    private fun file(ctx: Context, key: String) = File(dir(ctx), MessageDigest.getInstance("MD5").digest(key.toByteArray()).joinToString("") { "%02x".format(it) } + ".json")
    ...
    fun delete(ctx: Context, key: String) { file(ctx, key).delete() }
...
fun Candidate.toJson(): JSONObject = JSONObject()
    ...
    .put("areas", JSONArray(map.areas.map { a -> JSONObject().put("label", a.label).put("level", a.level.name).put("summary", a.summary).put("ring", JSONArray(a.ring.map { JSONArray(listOf(it.first, it.second)) })) }))
...
fun candidateFrom(j: JSONObject, input: Input): Candidate {
    ...
    val areas = j.getJSONArray("areas").objs().map { a -> MapArea(a.getString("label"), Level.valueOf(a.getString("level")), a.getString("summary"), a.getJSONArray("ring").map { i -> a.getJSONArray("ring").getJSONArray(i).let { it.getDouble(0) to it.getDouble(1) } }) }
